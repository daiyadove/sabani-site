export const translations = {
  ja: {
    hero: {
      title: '沖縄の伝統船サバニ',
      location: '石垣島の平久保半島に位置する',
      shipyard: '吉田サバニ造船',
      description1: 'ここ制作されたサバニで',
      description2: '平久保半島の雄大な自然を',
      description3: '満喫できます。',
      sailing1: '帆で風を受け、',
      sailing2: '海を走るサバニ。',
      activities1: '美しい珊瑚礁でシュノーケリング、',
      activities2: '海に沈む夕日を見ながら',
      activities3: 'サンセットクルージング。',
      memory1: 'あなたはこの旅で',
      memory2: '一生の思い出を',
      memory3: '手に入れるでしょう。',
      viewActivities: 'アクティビティを見る',
      reserve: '予約する'
    },
    activities: {
      title: '平久保サバニツアー',
      subtitle: 'サバニで巡る、石垣島の海の冒険',
      course1: {
        title: 'サバニライド',
        subtitle: 'Sabani ride',
        description: 'サバニライドで平久保半島の大自然を満喫コース。',
        features: [
          '船でしかいけないプライベートビーチに上陸',
          'ふかふかの砂のビーチ',
          '所要時間90分（海上50分）'
        ]
      },
      course2: {
        title: 'サバニライド＋スノーケル',
        subtitle: 'Sabani ride + Snorkel',
        description: 'サバニライドで平久保半島の珊瑚礁ポイントまで行き、スノーケルを楽しむコース。',
        features: [
          '生きた珊瑚礁でスノーケル',
          '色とりどりの珊瑚礁と熱帯魚',
          '所要時間120分（海上80分）'
        ]
      },
      course3: {
        title: 'サンセットクルーズ',
        subtitle: 'Sunset Cruise',
        description: '平久保半島の大自然を朱色に染めるサンセットを眺めるコース！',
        features: [
          '帆で静かに走るサバニからの夕日',
          'ロマンティックな景色',
          '所要時間100分',
          '日没時間の1時間前にお店に集合'
        ]
      },
      priceNote: '中学生以上は大人料金、小学生未満は2000円です。消費税・保険料込み。',
      adult: '大人',
      child: '小学生',
      reserveButton: '予約はこちら'
    },
    video: {
      title: '世代を超えた航海',
      description: '伝統的な航海術と現代のアドベンチャーが融合する特別な体験。私たちと一緒に、新しい冒険の扉を開きましょう。',
      note: '※実際のツアーの様子です。天候や季節により内容が異なる場合があります。'
    },
    menu: {
      home: 'ホーム',
      activities: 'アクティビティ',
      gallery: 'ギャラリー',
      guide: 'ガイド',
      video: '動画'
    },
    gallery: {
      title: 'フォトギャラリー',
      description: '実際の体験の様子をご覧ください。自然の中での特別な瞬間をお届けします。',
      alt1: '',
      alt2: '',
      alt3: '',
      alt4: '',
      alt5: '',
      alt6: ''
    },
    footer: {
      company: {
        title: '会社情報',
        name: '吉田サバニ造船',
        address: '〒907-0453 沖縄県石垣市川平1234-5',
        tel: 'TEL: 0980-XX-XXXX',
        email: 'Email: info@sabani.com'
      },
      links: {
        title: 'メニュー',
        home: 'ホーム',
        activities: 'アクティビティ',
        gallery: 'ギャラリー',
        guide: 'ガイド',
        video: '動画'
      },
      hours: {
        title: '営業時間',
        regular: '営業時間: 9:00 - 18:00',
        note: '※天候により営業時間が変更になる場合があります'
      },
      copyright: '© 2024 Sabani. All rights reserved.'
    },
    guide: {
      title: 'ご参加にあたって',
      description: '安全で楽しい体験のために、以下の内容をご確認ください。',
      items: [
        {
          title: '参加条件',
          description: '小学生以上の健康な方。泳げない方でもご参加いただけます。',
          icon: 'user'
        },
        {
          title: '服装と持ち物',
          description: `・乗船時は砂浜からの乗船で足元が濡れるため、濡れても良い服装
・サンダルでも乗船可能
・帽子（日差しが強いため）
・長袖やラッシュガード（日焼け対策）
・お飲み物
・酔い止め（必要な方）
・お子様用ライフジャケットや浮き輪（必要な方）
※泳げないお子様でもシュノーケルツアーに参加可能です`,
          icon: 'backpack'
        },
        {
          title: '当社で用意しているもの',
          description: `・ライフジャケット
・シュノーケルセット（ご自身のものも使用可）
・クバ笠（沖縄の伝統的な笠、希望者のみ）
・防水バッグ（カメラなどの撮影機材用、自己責任）
・タコメガネ（シュノーケルが苦手な方用）
※貴重品は預かりまたは車内保管可能`,
          icon: 'package'
        },
        {
          title: 'お支払い',
          description: '現地での現金支払い、または各種電子決済に対応しています。',
          icon: 'credit-card'
        },
        {
          title: 'ツアー中止基準',
          description: `サバニツアーは、天候の影響を強く受けます。
海のコンディションは刻一刻と変化していきます。
以下のことで、安全に航海できないと判断した場合、お客様の生命に関わることですので
ツアーを中止にすることがあることをご了承ください。

・警報発令中（大雨、洪水、暴風、津波、高潮）
・雷注意報発令中
・波高4.0m以上
・風速11.0m以上
・前線通過による急激な天候悪化が予想される場合
・現地ガイドが危険と判断した場合
・参加者の危険行動や指導に従えない場合
・参加者またはガイドの体調不良

※ツアー中止の連絡は、前日の時もあれば、当日の朝の海の状況を見てのご連絡といった場合もございます。
お客様のスケジュールの都合もございますので、なるべく早いご連絡を心がけています。`,
          icon: 'alert-triangle'
        }
      ]
    }
  },
  en: {
    hero: {
      title: 'Sabani - Traditional Okinawan Boat',
      location: 'Located in Hirakubo Peninsula, Ishigaki Island',
      shipyard: 'Yoshida Sabani Shipyard',
      description1: 'With our handcrafted Sabani,',
      description2: 'experience the magnificent nature',
      description3: 'of Hirakubo Peninsula.',
      sailing1: 'Catch the wind with the sail,',
      sailing2: 'glide through the ocean.',
      activities1: 'Snorkel in beautiful coral reefs,',
      activities2: 'watch the sunset over the ocean',
      activities3: 'on our sunset cruise.',
      memory1: 'Create unforgettable',
      memory2: 'memories',
      memory3: 'on this journey.',
      viewActivities: 'View Activities',
      reserve: 'Reserve Now'
    },
    activities: {
      title: 'Hirakubo Sabani Tour',
      subtitle: 'Ocean Adventures in Ishigaki Island by Sabani',
      course1: {
        title: 'Sabani Ride',
        subtitle: 'Sabani ride',
        description: 'Enjoy the nature of Hirakubo Peninsula on our Sabani boat.',
        features: [
          'Land on exclusive beach accessible only by boat',
          'Soft sandy beach',
          '90 minutes total (50 minutes on sea)'
        ]
      },
      course2: {
        title: 'Sabani Ride + Snorkel',
        subtitle: 'Sabani ride + Snorkel',
        description: 'Ride to coral reef points of Hirakubo Peninsula and enjoy snorkeling.',
        features: [
          'Snorkel in living coral reefs',
          'Colorful coral reefs and tropical fish',
          '120 minutes total (80 minutes on sea)'
        ]
      },
      course3: {
        title: 'Sunset Cruise',
        subtitle: 'Sunset Cruise',
        description: 'Watch the sunset paint Hirakubo Peninsula in crimson colors!',
        features: [
          'Sunset view from sail-powered Sabani',
          'Romantic scenery',
          '100 minutes total',
          'Meet at the shop 1 hour before sunset'
        ]
      },
      priceNote: 'Adult price applies from junior high school students. Children under elementary school age: ¥2,000. Tax and insurance included.',
      adult: 'Adult',
      child: 'Elementary',
      reserveButton: 'Reserve Now'
    },
    video: {
      title: 'A Journey Across Generations',
      description: 'A special experience that combines traditional navigation and modern adventure. Join us in opening new doors to adventure.',
      note: '* Actual tour footage. Content may vary depending on weather and season.'
    },
    menu: {
      home: 'Home',
      activities: 'Activities',
      gallery: 'Gallery',
      guide: 'Guide',
      video: 'Video'
    },
    gallery: {
      title: 'Photo Gallery',
      description: 'Take a look at our actual experiences. We deliver special moments in nature.',
      alt1: '',
      alt2: '',
      alt3: '',
      alt4: '',
      alt5: '',
      alt6: ''
    },
    footer: {
      company: {
        title: 'Company Info',
        name: 'Yoshida Sabani Shipyard',
        address: '1234-5 Kabira, Ishigaki City, Okinawa 907-0453',
        tel: 'TEL: 0980-XX-XXXX',
        email: 'Email: info@sabani.com'
      },
      links: {
        title: 'Menu',
        home: 'Home',
        activities: 'Activities',
        gallery: 'Gallery',
        guide: 'Guide',
        video: 'Video'
      },
      hours: {
        title: 'Business Hours',
        regular: 'Hours: 9:00 AM - 6:00 PM',
        note: '* Business hours may change depending on weather conditions'
      },
      copyright: '© 2024 Sabani. All rights reserved.'
    },
    guide: {
      title: 'Participation Guide',
      description: 'Please check the following information for a safe and enjoyable experience.',
      items: [
        {
          title: 'Requirements',
          description: 'Healthy individuals from elementary school age. Non-swimmers are welcome.',
          icon: 'user'
        },
        {
          title: 'Clothing and Items to Bring',
          description: `・Clothes that can get wet (boarding from beach)
・Sandals are acceptable
・Hat (strong sunlight)
・Long sleeves or rash guard (sun protection)
・Drinks
・Motion sickness medicine (if needed)
・Life jacket or float for children (if needed)
※Non-swimming children can join snorkeling tour`,
          icon: 'backpack'
        },
        {
          title: 'Equipment We Provide',
          description: `・Life jackets
・Snorkeling set (personal equipment also welcome)
・Traditional Okinawan Kuba hat (optional)
・Waterproof bags (for cameras, at own risk)
・Viewing goggles (for those not comfortable with snorkeling)
※Valuables can be stored safely`,
          icon: 'package'
        },
        {
          title: 'Payment',
          description: 'We accept cash on-site and various electronic payment methods.',
          icon: 'credit-card'
        },
        {
          title: 'Tour Cancellation Criteria',
          description: `Sabani tours are highly weather-dependent.
Sea conditions can change rapidly from moment to moment.
Please understand that we may need to cancel tours for your safety if we determine that safe navigation is not possible under the following conditions:

・Weather warnings (heavy rain, flood, storm, tsunami, storm surge)
・Thunder advisory
・Wave height over 4.0m
・Wind speed over 11.0m/s
・Approaching weather front with expected deterioration
・Guide's safety assessment
・Dangerous behavior or non-compliance
・Poor health condition of participants or guide

※Tour cancellation notices may be given the day before or on the morning of the tour based on sea conditions.
We strive to notify you as early as possible, considering your schedule arrangements.`,
          icon: 'alert-triangle'
        }
      ]
    }
  }
}
